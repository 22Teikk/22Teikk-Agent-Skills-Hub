...
```
/teikk-review → code-reviewer (with code-review-and-quality skill) → report
```

**Use when:** the same single-persona invocation happens repeatedly with the same setup.

**Examples in this repo:** `/teikk-review`, `/teikk-test`, `/teikk-code-simplify`.
...
**Examples in this repo:** `/teikk-ship`.
...
```
user runs:  /teikk-spec  →  /teikk-planning  →  /teikk-build  →  /teikk-test  →  /teikk-review  →  /teikk-ship
```
...
In Claude Code, parallel fan-out (Pattern 3) requires issuing **multiple Agent tool calls in a single assistant turn**. Sequential turns serialize execution. `/teikk-ship` calls this out explicitly. Any new orchestrator command should do the same.

---

## Worked example: Agent Teams for competing-hypothesis debugging

This example shows when to reach for **Agent Teams** instead of `/teikk-ship`'s subagent fan-out. The two patterns look similar from a distance — both spawn the same three personas — but the value comes from a different place.
...
A single agent will pick the first plausible theory and stop investigating. A `/teikk-ship`-style subagent fan-out would have each persona report independently — but their reports never meet, so nothing rules out the wrong theories.
...
### Why this is *not* a `/teikk-ship` job

| | `/teikk-ship` (subagents) | Agent Teams |
|--|--------------------|-------------|
...
`/teikk-ship` is a verdict; Agent Teams is an investigation.
...
Three Sonnet teammates running for ~10–15 minutes of investigation costs noticeably more than the same three personas spawned as subagents by `/teikk-ship`. The justification is *quality of conclusion* — for production debugging where the wrong fix is expensive, the extra tokens are a bargain. For a routine PR review, stick with `/teikk-ship`.
...
- Production-bound verdict on a known diff → use `/teikk-ship` (subagents).
...
- The user already knew they wanted a review; they could have called `/teikk-review` directly
...
An agent that calls `/teikk-spec`, then `/teikk-planning`, then `/teikk-build`, etc. on the user's behalf.
...
`/teikk-ship` calls a `pre-ship-coordinator` that calls a `quality-coordinator` that calls `code-reviewer`.
