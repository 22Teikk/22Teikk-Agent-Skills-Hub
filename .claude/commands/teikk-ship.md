...
6. **E2E (optional)** — If `.maestro/flows/` exists and SPEC does not say `E2E: none`, follow the teikk-agents-skills:android-e2e-maestro skill.: run `maestro test .maestro/flows/`. Fail → Recommended fix or Blocker if SPEC declares E2E required. If no `.maestro/` dir, skip silently.
