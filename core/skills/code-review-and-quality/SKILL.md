...
version: 1.0.0
platform: generic
depends-on: [security-and-hardening]
---
...
For detailed profiling and optimization, see the `android-performance-auditor` agent. Does the change introduce performance problems?

- Any database or network I/O running synchronously on the Main thread?
- Any memory leaks (e.g. unclosed RxJava subscriptions or unreleased contexts)?
- Any unnecessary recompositions or layout weights causing jank?
...
4. Does it have known vulnerabilities? (Check dependency check/security reports)
