...
/**
 * Like copyRelative, but the package-side source path and the project-side
 * destination path differ — used to merge core/skills + packs/<platform>/skills
 * (and the agents equivalents) into the flat project-side `skills/`/`agents/`
 * destination. Calling this once per source with the SAME destRelPath merges
 * every source into that one destination directory (copyOrMerge recurses into
 * whatever is already there rather than overwriting it).
 */
function copyMapped(packageRoot, projectRoot, srcRelPath, destRelPath, opts = {}) {
  const absSrc = path.join(packageRoot, srcRelPath);
  const absDest = path.join(projectRoot, destRelPath);
  const skipped = opts.skipped || [];
  const written = opts.written || [];
  const ownedFiles = opts.ownedFiles || new Set();

  copyOrMerge(absSrc, absDest, destRelPath, ownedFiles, written, skipped);
  return destRelPath;
}
