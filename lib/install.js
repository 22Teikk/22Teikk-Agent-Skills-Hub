...
function findPackageRoot(startDir) {
  ...
  while (true) {
    ...
    if (fs.existsSync(pkg)) {
      try {
        ...
        if (data.name === 'agent-skills' && fs.existsSync(path.join(dir, 'skills'))) {
          return dir;
        }
      } catch {
        // keep walking
  ...
  throw new Error(
    'Could not locate agent-skills package. Run from a project with agent-skills installed, or use --package-root.',
  );
}
...
function install({ projectRoot, packageRoot, targetInput, version }) {
  ...
  for (const relativePath of mergeCopyPaths(targets)) {
    copied.push(copyRelative(packageRoot, projectRoot, relativePath));
  }

  if (needsGeminiSkills(targets)) {
    copied.push(copyGeminiSkills(packageRoot, projectRoot));
  }

  for (const link of mergeSymlinks(targets)) {
    copied.push(ensureSymlink(projectRoot, link.linkPath, link.targetPath));
  }
  ...
  return { targets, copied, gitignorePatterns: buildPatterns(allTargets) };
}
...
function uninstall({ projectRoot }) {
  ...
  for (const relativePath of copyPaths) {
    if (removeRelative(projectRoot, relativePath)) {
