...
**3. Spec — `.teikk/SPEC.md` present and structurally complete**
Check whether `.teikk/SPEC.md` exists. If it does, confirm all eight required sections are present: Objective, Tech Stack, Architecture, Observability, Commands, Testing Strategy (with Traceability Matrix), Boundaries.
- Pass: file present, all sections found
- Warn: file present but one or more sections missing (list which)
- Fail: file absent
- Next action on fail: run `/teikk-spec`.

**4. PROJECT.yaml — `.teikk/PROJECT.yaml` present**
Check whether `.teikk/PROJECT.yaml` exists.
- Pass: file present
- Warn: absent (was the spec created before v2.3? Re-run `/teikk-spec` or create manually)
- No fail — this is a non-blocking warn.

**5. Platform: Android — mobile-mcp MCP server**
Only run if `.teikk/PROJECT.yaml` exists and `platforms` includes `android`, OR if `.teikk/SPEC.md` mentions `Android` or `Kotlin` in the Tech Stack. Check whether `mobile-mcp` is listed in `.claude/settings.json` or `.claude/settings.local.json` under `mcpServers`.
- Pass: listed
- Warn: Android project but `mobile-mcp` not configured (UI/UX testing via `/teikk-qa` will not work)
- Skip: non-Android project

**6. E2E tooling**
Only run if `.teikk/PROJECT.yaml` has `e2e:` set to something other than `none`, OR if `.teikk/SPEC.md` has `E2E:` set to `Maestro`, `XCUITest`, or `integration_test`.
...
| 4 | PROJECT.yaml present | ✓/✗/⚠ | ... |
| 5 | mobile-mcp (Android) | ✓/⚠/skip | ... |
| 6 | E2E tooling | ✓/⚠/skip | ... |
| 7 | Tasks plan + todo | ✓/⚠/skip | ... |
| 8 | Git working tree | ✓/⚠ | ... |
