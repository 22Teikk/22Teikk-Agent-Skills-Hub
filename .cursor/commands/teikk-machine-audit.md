...
1. Collect evidence — MCP servers and scope (`claude mcp list` + `claude mcp get <server>` for each), `~/.claude/settings.json`, `~/.claude/CLAUDE.md`, whether the current project actually needs each global server, last session's cache-read/cache-write vs input/output cost ratio if available.
2. Apply red flags — user-scoped MCP with no matching project artifact, broad-matcher hooks with no project-existence guard, duplicate hooks on the same event, a globally-pinned high reasoning-effort default, dead/failed MCP connections, mandatory-workflow injections with no project scoping.
3. Quantify — if a cost breakdown exists, check whether cache-read + cache-write together exceed ~50-60% of total cost (signals oversized/repeatedly-rewritten base context, not code generation).
